...
// Basic Auth 검증 함수
static bool basic_authenticate(const httplib::Request &req, httplib::Response &res)
{
    static std::string auth_user = get_env_var("LLAMA_AUTH_USER", "admin");
    static std::string auth_pass = get_env_var("LLAMA_AUTH_PASS", "securepassword");

    if (!req.has_header("Authorization"))
    {
        res.status = 401;
        res.set_header("WWW-Authenticate", "Basic realm=\"LlamaServer\"");
        res.set_content("Unauthorized", "text/plain");
        return false;
    }

    std::string auth_header = req.get_header_value("Authorization");
    std::string expected_auth = "Basic " + httplib::detail::base64_encode(auth_user + ":" + auth_pass);
...
static std::string process_prompt(const std::string &prompt, Translator &translator)
{
    if (!translator.is_korean(prompt))
    {
        return prompt; // 번역이 필요 없는 경우 원본 유지
    }

    LOG_INF("🔄 Translating Korean prompt to English...\n");
    ...
    LOG_INF("✅ Translated Prompt: %s\n", translated_text.c_str());

    // 3. 번역된 텍스트에 코드 블록 추가 (올바른 위치 유지)
    std::string final_prompt;
    size_t code_index = 0;

    std::stringstream ss(translated_text);
    std::string line;
    while (std::getline(ss, line))
    {
        final_prompt += line + "\n";

        // 원본 텍스트에서 코드가 있던 위치에 코드 블록 추가
        if (code_index < code_blocks.size() && line.empty())
        {
            final_prompt += code_blocks[code_index++] + "\n";
        }
    }
...
int main(int argc, char **argv)
{
    ...
    auto middleware_validate_api_key = [&params, &res_error](const httplib::Request &req, httplib::Response &res)
    {
        ...
        // Basic Auth 우선 적용
        if (!basic_authenticate(req, res))
        {
            return false; // 인증 실패 시 종료
        }
    ...
    svr->set_pre_routing_handler([&middleware_validate_api_key, &middleware_server_state](const httplib::Request &req,
        ...
        if (req.method == "OPTIONS") {
            ...
            return httplib::Server::HandlerResponse::Handled;  // OPTIONS 요청은 추가 인증 없이 처리
        }
    ...
    // handle completion-like requests (completion, chat, infill)
    const auto handle_completions_impl = [&ctx_server, &res_error, &res_ok](server_task_type type, json &data,
                                                                            ...
                                                                            oaicompat_type oaicompat)
    {
        ...
        Translator translator; // Translator 객체 생성
        auto completion_id = gen_chatcmplid();
        std::vector<server_task> tasks;

        try
        {
            auto &prompt = data.at("prompt"); // 프롬프트 추출

            // // 🟢 프롬프트가 한국어이면 영어로 변환
            if (translator.is_korean(prompt))
            {
                data["prompt"] = process_prompt(prompt, translator);
            }

            std::cout << "FINAL PROMPT: " << data["prompt"] << std::endl;

            // 기존 로직 유지
            std::vector<llama_tokens> tokenized_prompts = tokenize_input_prompts(ctx_server.vocab, data["prompt"], true, true);
    ...
    const auto handle_infill = [&ctx_server, &res_error, &handle_completions_impl](const httplib::Request &req,
                                                                                   httplib::Response &res)
    {
        ...
        {
            res_error(res,
                      format_error_response(string_format("Infill is not supported by this model: %s", err.c_str()),
                                            ERROR_TYPE_NOT_SUPPORTED));
            return;
        }
        ...
        return handle_completions_impl(SERVER_TASK_TYPE_INFILL, data, req.is_connection_closed, res,
                                       OAICOMPAT_TYPE_NONE); // infill is not OAI compatible
    };

    const auto handle_chat_completions = [&ctx_server, &params, &res_error, &handle_completions_impl](
